...
$previousMonthTime = strtotime('first day of last month');
$monthName = date('F Y', $previousMonthTime);
$monthQuery = date('Y-m', $previousMonthTime);
...
try {
    // Process summaries directly since the workflow scheduler handles the timing (runs on the 1st of each month)
    ...
    $response['actions'][] = "Month summaries processed. Emails sent: {$emailsSent}, Pushes triggered: {$pushesSent}";
