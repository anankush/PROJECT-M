...
function renderTabs() {
    ...
    categories.forEach(cat => {
        ...
        li.innerHTML = `<button class="category-tab ${currentCategoryId === cat.id ? 'active' : ''}" style="flex:1;"><i class="fas fa-folder"></i> <span style="flex:1; text-align:left; overflow:hidden; text-overflow:ellipsis;">${safeName}</span></button>`;
        li.querySelector('.category-tab').addEventListener('click', () => loadCategory(cat.id, cat.category_name, true));
...
function formatRecordDetailValue(value, type = 'text') {
    if (value === null || value === undefined || value === '') return '-';
    if (type === 'currency') return `${escapeHtml(userCurrency)}${escapeHtml(value)}`;
    if (type === 'time') return escapeHtml(formatTime12Hour(String(value)));
    return escapeHtml(value);
}

function showRecordDetails(row) {
    const details = [
        ['Date', escapeHtml(row.entry_date)],
        ['Time', formatRecordDetailValue(row.entry_time, 'time')],
        ['Amount', `${escapeHtml(userCurrency)}${escapeHtml(row.amount)}`],
        ['Description', formatRecordDetailValue(row.description)]
    ];

    Object.entries(row.custom_data || {}).forEach(([key, fieldData]) => {
        const type = typeof fieldData === 'object' && fieldData !== null ? fieldData.type : 'text';
        const value = typeof fieldData === 'object' && fieldData !== null ? fieldData.value : fieldData;
        details.push([key, formatRecordDetailValue(value, type)]);
    });

    details.push(['Saved At', escapeHtml(row.created_at)]);
    const detailRows = details.map(([label, value]) => `<div class="record-detail-item"><span class="record-detail-label">${escapeHtml(label)}</span><span class="record-detail-value">${value}</span></div>`).join('');
    const actionHtml = `<div class="record-detail-actions"><button type="button" class="record-detail-action edit" id="recordDetailEdit"><i class="fas fa-pen"></i><span>Edit</span></button><button type="button" class="record-detail-action delete" id="recordDetailDelete"><i class="fas fa-trash-alt"></i><span>Delete</span></button></div>`;

    Swal.fire({
        title: 'Record Details',
        html: `<div class="record-detail-popup"><div class="record-detail-list">${detailRows}</div>${actionHtml}</div>`,
        showConfirmButton: false,
        showCloseButton: true,
        width: 560,
        customClass: { popup: 'record-detail-swal' },
        didOpen: () => {
            document.getElementById('recordDetailEdit').addEventListener('click', () => {
                Swal.close();
                editRecord(row);
            });
            document.getElementById('recordDetailDelete').addEventListener('click', () => {
                Swal.close();
                deleteRecord(row.id);
            });
        }
...
function addCustomFieldRow(key = '', val = '', readonly = false, type = 'text') {
    ...
    const allowedTypes = ['text', 'number', 'currency', 'date', 'time'];
    ...
    if (readonly) {
        div.innerHTML = `<div class="swal-field"><label class="swal-label">${safeKey} ${type === 'currency' ? `(${escapeHtml(userCurrency)})` : ''}</label><div style="display:flex; gap:10px; align-items:center;"><input type="hidden" class="cf-key" value="${safeKey}"><input type="hidden" class="cf-type" value="${type}"><input type="${inputType}" ${stepAttr} class="cf-val theme-input-select swal-input" placeholder="Value" value="${safeValue}" style="flex:1;"><i class="fas fa-times cf-remove" onclick="confirmRemoveCustomField('${div.id}')" style="cursor:pointer; color:var(--danger); font-size:1.2rem; padding:10px;"></i></div></div>`;
...
function getFormHtml(data = null) {
    ...
    return `<div class="swal-form-container">
                <div class="swal-field"><label class="swal-label">Date</label><input id="f-date" type="date" class="theme-input-select swal-input" value="${escapeHtml(date)}" onkeydown="return false" onchange="checkDateMonth(this.value)" onclick="this.showPicker()"></div>
                <div id="date-hint" style="font-size:0.8rem; color:#f59e0b; display:none; text-align:left; margin-top:-10px;"></div>
                <div class="swal-field"><label class="swal-label">Time</label><input id="f-time" type="time" class="theme-input-select swal-input" value="${escapeHtml(timeVal)}" onkeydown="return false" onclick="this.showPicker()"></div>
                <div class="swal-field"><label class="swal-label">Amount (${escapeHtml(userCurrency)})</label><input id="f-amt" type="number" step="0.01" class="theme-input-select swal-input" value="${escapeHtml(amt)}"></div>
                <div class="swal-field"><label class="swal-label">Description</label><input id="f-desc" type="text" class="theme-input-select swal-input" value="${escapeHtml(desc)}" maxlength="255"></div>
